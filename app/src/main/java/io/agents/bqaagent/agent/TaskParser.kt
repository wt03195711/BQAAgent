...
object TaskParser {
    ...
    private fun matchScreenshot(lower: String): ParseResult? {
        ...
        if (isNegatedScreenshotRequest(lower)) return null
        return ParseResult(
            action = "screenshot",
            intent = null,
    ...
    private fun isNegatedScreenshotRequest(lower: String): Boolean {
        return listOf(
            "no screenshot",
            "no screenshots",
            "without screenshot",
    ...
    private fun matchOpenApp(lower: String, original: String, installedPackages: List<String>): ParseResult? {
        val match = OPEN_APP_PATTERN.find(original.trim()) ?: return null
        ...
        if (looksLikeComplexOpenAppTask(lower, appNameLower)) return null
    ...
    private fun looksLikeComplexOpenAppTask(lower: String, appNameLower: String): Boolean {
        if (listOf(" and ", " then ", " after ", " if ", " otherwise ").any { lower.contains(it) }) return true
        if (listOf(",", ";", "，", "；", "。").any { lower.contains(it) }) return true
        return listOf(
            " check ",
            " search ",
            " find ",
