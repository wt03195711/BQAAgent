...
package io.agents.bqaagent.server
 ...
 * Provides an H5 page for configuring the local app from a desktop browser.
 */
class ConfigServer(
    ...
    private fun handleGetScreenFull(): Response {
        if (!io.agents.bqaagent.adb.LocalAdbDeviceDriver.awaitReady(2_000L)) {
            return corsResponse(
                newFixedLengthResponse(
                    Response.Status.OK, MIME_JSON,
                    """{"code":-1,"message":"Local ADB is not ready"}"""
                )
            )
        }
        val tree = io.agents.bqaagent.adb.LocalAdbDeviceDriver.getScreenTreeFull()
        val data = JsonObject().apply {
