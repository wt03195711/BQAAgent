...
public class AnalyzeScreenVisualTool extends BaseTool {
    ...
    public String getDescriptionEN() {
        return "Take a screenshot of the current screen and analyze it using a vision model. "
                ...
                + "1. get_screen_info returns SCREEN_TREE_UNUSABLE (ADB cannot read the UI tree) → call this tool IMMEDIATELY.\n"
                + "2. get_screen_info returns data, but the target element is NOT in the ADB data → call this tool BEFORE guessing or clicking unrelated elements.\n"
                + "3. Do NOT try to reason through incomplete ADB data — use this tool first.\n\n"
    ...
    public String getDescriptionCN() {
        return "截取当前屏幕并使用视觉模型进行分析，返回建议的下一步操作及像素坐标。\n\n"
                + "使用时机（优先级）：\n"
                + "1. get_screen_info 返回 SCREEN_TREE_UNUSABLE（ADB 无法读取 UI 树）→ 立即调用此工具。\n"
                + "2. get_screen_info 返回了数据，但目标元素不在 ADB 数据中 → 在猜测或点击不相关元素之前，先调用此工具。\n"
                + "3. 不要尝试通过不完整的 ADB 数据进行推理——优先使用此工具。\n\n"
                + "\"intent\" 参数必须描述：\n"
                + "1. 当前任务目标\n"
                + "2. 上一步尝试的操作及结果\n"
                + "3. 需要从视觉分析中获取的信息\n\n"
