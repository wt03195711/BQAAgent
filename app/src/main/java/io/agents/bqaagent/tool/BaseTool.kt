...
abstract class BaseTool {
    ...
    fun getParametersWithWaitAfter(): List<ToolParameter> {
        ...
        if (getName() !in listOf("wait", "finish", "get_screen_info", "take_screenshot", "get_installed_apps", "find_node_info", "scroll_to_find", "list_scheduled_tasks", "schedule_task", "cancel_scheduled_task", "analyze_screen_visual", "analyze_user_image")) {
    ...
    /**
     * Local ADB can briefly be connecting. Tools should tolerate that
     * short gap instead of failing immediately.
     */
    @JvmOverloads
    protected fun requireDeviceDriver(timeoutMs: Long = 20_000L): LocalAdbDeviceDriver? {
        return if (LocalAdbDeviceDriver.awaitReady(timeoutMs)) LocalAdbDeviceDriver else null
    }
