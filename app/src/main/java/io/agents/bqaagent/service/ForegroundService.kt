...
class ForegroundService : Service() {
    ...
    companion object {
        ...
        const val CHANNEL_ID = "BQAAgent_foreground_channel"
        const val NOTIFICATION_ID = 1001
        private const val EXTRA_TITLE = "extra_title"
        private const val EXTRA_TEXT = "extra_text"
        private const val DEFAULT_TASK_TITLE = "BQAAgent · Task in progress"
        private const val DEFAULT_TASK_TEXT = "Running task..."
        private const val DEFAULT_MONITOR_TITLE = "BQAAgent · Monitoring"
        private const val DEGRADED_MONITOR_TITLE = "BQAAgent · Monitoring paused"
        ...
        fun showMonitorStatus(context: Context): Boolean {
            ...
            if (capabilities.localAdbState != ServiceBindingState.READY) {
                return showNotification(
                    context,
                    DEGRADED_MONITOR_TITLE,
                    if (capabilities.localAdbState == ServiceBindingState.CONNECTING) {
                        "Local ADB connecting..."
                    } else {
                        "Local ADB disconnected"
                    }
                )
            }
        ...
        private fun buildNotification(context: Context, title: String, text: String): Notification {
            val intent = Intent(context, io.agents.bqaagent.ui.chat.ComposeChatActivity::class.java)
