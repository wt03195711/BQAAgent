...
package io.agents.bqaagent.debug
 ...
 *
 * Usage:
 *   adb shell am broadcast -a io.agents.bqaagent.DEBUG_TASK --es task "open my camera"
 *
 * Set DeepSeek Cloud LLM config:
 *   adb shell am broadcast -a io.agents.bqaagent.DEBUG_TASK --es task "config:" \
 *     --es api_key "sk-..." --es model_name "deepseek-v4-flash"
 *
 * With custom base URL:
 *   adb shell am broadcast -a io.agents.bqaagent.DEBUG_TASK --es task "config:" \
 *     --es api_key "sk-..." --es provider "CUSTOM" --es base_url "https://api.deepseek.com" --es model_name "deepseek-v4-pro"
 */
class DebugTaskReceiver : BroadcastReceiver() {
    override fun onReceive(context: Context, intent: Intent) {
        if (!io.agents.bqaagent.BuildConfig.DEBUG) return
        ...
        if (supportAction.isNotEmpty()) {
            runAsync("debug-support-$supportAction") {
                handleSupportDebug(context.applicationContext, intent, supportAction)
            }
            return
        }
        ...
        if (task.startsWith("config:")) {
            try {
                ...
                val provider = intent.getStringExtra("provider") ?: CloudProvider.DEEPSEEK.name
                if (provider == "LOCAL") {
                    ...
                    val resolvedBaseUrl = baseUrl
                        ?: CloudProvider.findProviderForModel(modelName ?: "")?.defaultBaseUrl
                        ?: CloudProvider.DEEPSEEK.defaultBaseUrl
                ...
                val vm = io.agents.bqaagent.ClawApplication.appViewModelInstance
    ...
    private fun executeTool(intent: Intent, toolName: String, paramsJson: String) {
        try {
            ...
            breadcrumb(io.agents.bqaagent.ClawApplication.instance, "executeTool start tool=$toolName params=$params")
            ...
            if (result.isSuccess) {
                ...
                breadcrumb(io.agents.bqaagent.ClawApplication.instance, "executeTool success tool=$toolName data=${result.data}")
            } else {
                XLog.w("DebugTaskReceiver", "Debug tool failed: $toolName -> ${result.error}")
                breadcrumb(io.agents.bqaagent.ClawApplication.instance, "executeTool fail tool=$toolName error=${result.error}")
            }
        } catch (e: Exception) {
            XLog.e("DebugTaskReceiver", "Failed to execute debug tool", e)
            breadcrumb(io.agents.bqaagent.ClawApplication.instance, "executeTool exception tool=$toolName error=${e.message}")
        }
    }
    ...
    private fun simulateIncomingMessage(intent: Intent, simulateMessage: String) {
        try {
            ...
            breadcrumb(
                io.agents.bqaagent.ClawApplication.instance,
        ...
        try {
            when (action.lowercase()) {
                "status" -> {
                    ...
                    breadcrumb(io.agents.bqaagent.ClawApplication.instance, "backend status ${LocalBackendHealth.debugStateSummary()}")
                }
                "force_cpu_safe" -> {
                    ...
                    breadcrumb(io.agents.bqaagent.ClawApplication.instance, "backend force_cpu_safe reason=$reason ${LocalBackendHealth.debugStateSummary()}")
                }
                "clear_cpu_safe" -> {
                    ...
                    breadcrumb(io.agents.bqaagent.ClawApplication.instance, "backend clear_cpu_safe ${LocalBackendHealth.debugStateSummary()}")
                }
                "mark_pending_gpu_init" -> {
                    ...
                    breadcrumb(io.agents.bqaagent.ClawApplication.instance, "backend mark_pending_gpu_init model=$modelPath ${LocalBackendHealth.debugStateSummary()}")
                }
                "clear_pending_gpu_init" -> {
                    ...
                    breadcrumb(io.agents.bqaagent.ClawApplication.instance, "backend clear_pending_gpu_init ${LocalBackendHealth.debugStateSummary()}")
                }
                "recover_pending_gpu_crash" -> {
                    ...
                    breadcrumb(io.agents.bqaagent.ClawApplication.instance, "backend recover_pending_gpu_crash recovered=$recovered ${LocalBackendHealth.debugStateSummary()}")
                }
                else -> {
                    XLog.w("DebugTaskReceiver", "Unknown backend_action=$action")
                    breadcrumb(io.agents.bqaagent.ClawApplication.instance, "backend unknown action=$action")
                }
            }
        } catch (e: Exception) {
            XLog.e("DebugTaskReceiver", "Failed backend_action=$action", e)
            breadcrumb(io.agents.bqaagent.ClawApplication.instance, "backend exception action=$action error=${e.message}")
        }
    }

    private fun handleSupportDebug(context: Context, intent: Intent, action: String) {
        try {
            when (action.lowercase()) {
                "build_debug_report" -> {
                ...
                "local_adb_connect" -> {
                    val host = firstNonBlank(
                        ...
                        intent.getStringExtra("connect_host"),
                    ) ?: "127.0.0.1"
                    val port = intExtra(intent, "port", "connect_port", "local_adb_port")
                    if (port == null || port <= 0) {
                        XLog.w("DebugTaskReceiver", "local_adb_connect missing valid port")
                        breadcrumb(context, "support local_adb_connect invalid port")
                        return
                    }

                    LocalAdbAutomation.init(context)
                    val result = LocalAdbAutomation.configureConnection(host, port)
                    val output = result.combinedOutput.ifBlank { "exit=${result.exitCode}" }
                    if (result.isSuccess) {
                        XLog.i("DebugTaskReceiver", "Local ADB debug connect success host=$host port=$port output=$output")
                        breadcrumb(context, "support local_adb_connect success host=$host port=$port output=$output")
                    } else {
                        XLog.w("DebugTaskReceiver", "Local ADB debug connect failed host=$host port=$port output=$output")
                        breadcrumb(context, "support local_adb_connect failed host=$host port=$port output=$output")
                    }
                }
                else -> {
